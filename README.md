...
### Live Codex provider E2E prerequisites (destructive)

`verify:codex-subscription-provider:e2e` is an interactive, non-CI functional harness and is excluded from ordinary verification. Every prerequisite below is required:

- Begin with an exact clean checkout, run the canonical `pnpm dist`, and leave its `release/` candidate unchanged. That command builds the runtime, release tree, packaged Windows candidate, and installer required by this harness. Set `PRIME_CONTINUIM_CODEX_E2E_INSTALLED_EXE` to an operator-supplied external `Prime Continuim.exe` outside this repository. Before creating state and again after the final desktop close, the harness requires the external executable, `app.asar`, hostd, and runtime pointer to byte-match that exact candidate and correlates the current installer checksum. The application version comes from the integrity-bound packaged manifest. This correlation does not prove that NSIS performed the installation.
- Use a disposable Windows x64 VM/checkpoint, an interactive `en-US` desktop, and only the dedicated medium-integrity account `PrimeCodexE2E`. Inbox Windows PowerShell and Windows UI Automation must be available to invoke the exact process's unique enabled, on-screen native **Close** button during Electron restart.
- Provide a working system browser and provider network. The app-server account must initially report signed out; the harness cannot observe whether the dedicated account's system browser already has a web session. No ambient `CODEX_HOME` or keyring selector is injected.
- Set `PRIME_CONTINUIM_CODEX_E2E_DISPOSABLE_CHECKPOINT=DISPOSABLE_WINDOWS_CHECKPOINT_READY`, then run `pnpm verify:codex-subscription-provider:e2e -- --i-understand-this-will-use-and-sign-out-my-chatgpt-subscription --disposable-windows-checkpoint` and type the exact authorization displayed. Never run it from a normal workstation account or without the disposable checkpoint.

The harness uses a canonical isolated host/app-data root and a disclosed `production_store_fixture`; that setup is not native-picker evidence. It launches the byte-correlated external candidate with a privileged TCP-free DevTools pipe, drives sign-in, Run, Stop, and sign-out only through visible renderer controls, and uses production account/conversation read-reconciliation calls for evidence. It can prove a streamed completed turn, an exact interrupted turn, and an Electron-only restart against the same detached hostd incarnation. After restart it requires the initial recovered durable conversation plus two further identical durable observations, each separated by at least the renderer's four-second terminal poll interval; authority remains identical, revisions are monotonic, and all three observations contain exactly the same two admitted user operations. This proves only that no additional durable admission was observed across two renderer-interval-sized observation gaps. It does not prove that renderer poll events occurred in those gaps or establish a provider RPC count. It also proves a signed-out final app state and absence of `auth.json` within the canonical protected Codex home.

A functionally successful run intentionally emits `functional_passed_cleanup_required` with `cleanup.status: cleanup_required` and exits with status 2. The detached hostd/app-server and retained fixture are deliberately not killed or deleted; an external VM controller or operator must roll back or destroy the disposable environment. The receipt is not cleanup-completion evidence, not native-picker evidence, and not release-readiness evidence. It also is not IPC sender-trust/security, ordinary-user authority, hostd-restart, installed-lifecycle, or signing evidence. No authenticated run is cited here.
