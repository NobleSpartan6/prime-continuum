...
- Prime Agent browser OAuth is a local-only, digest-bound durable attempt rather than a replayable session RPC. Hostd publishes the attempt family only after its private journal has recovered under endpoint ownership; dynamic new-start eligibility is separate and is withdrawn on unresolved helper custody, provider/runtime loss, capacity, or journal uncertainty. Desktop persists before the sole start call, reconciles response loss and restart with read-only status, and never replays login or browser launch. Terminal cancellation requires positive helper/run retirement, and acknowledgement advances the exact terminal revision once. The durable journal and durable record projection exclude secrets, authorization URLs, account data, raw failures, process output, and paths; a pinned authorization URL may cross only the transient trusted-local host-to-main live envelope, and the renderer receives only a sanitized interaction state. This source contract is not authenticated-provider execution evidence.
