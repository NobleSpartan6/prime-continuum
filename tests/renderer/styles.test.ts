...
describe('renderer style contracts', () => {
  ...
  it('keeps the workbench shell quiet, fit-safe, and operable', async () => {
    const css = await readFile(resolve('src/renderer/src/styles.css'), 'utf8')

    expect(css).toMatch(/\.topbar\s*{[^}]*border-block-end:\s*0;/s)
    expect(css).toMatch(/\.topbar__leading\s*{[^}]*overflow:\s*clip;[^}]*border-inline-end:\s*0;[^}]*container-name:\s*topbar-leading;[^}]*container-type:\s*inline-size;/s)
    expect(css).toMatch(/\.sidebar\s*{[^}]*border-inline-end:\s*0;/s)
    expect(css).toMatch(/\.sidebar__footer\s*{[^}]*border-block-start:\s*0;/s)
    expect(css).toMatch(/\.topbar\s*{[^}]*background:\s*var\(--color-surface\);[^}]*box-shadow:\s*none;/s)
    expect(css).toMatch(/\.topbar \.task-state\s*{[^}]*inline-size:\s*2\.25rem;[^}]*block-size:\s*2\.25rem;[^}]*justify-content:\s*center;[^}]*padding:\s*0;/s)
    expect(css).toMatch(/\.topbar \.task-state__label\s*{[^}]*display:\s*none;/s)
    expect(css).toMatch(/@container topbar-leading \(max-width:\s*14\.5rem\)\s*{\s*\.topbar__brand-name\s*{[^}]*display:\s*none;/s)
    expect(css).toMatch(/@container topbar-leading \(max-width:\s*19rem\)\s*{\s*:root\[data-native-platform="darwin"\] \.topbar__brand-name\s*{[^}]*display:\s*none;/s)
    expect(css).toMatch(/@media \(max-width:\s*50rem\)[\s\S]*?:root\[data-native-platform="darwin"\] \.topbar__leading\s*{[^}]*inline-size:\s*7\.75rem;[^}]*min-inline-size:\s*7\.75rem;[^}]*padding-inline:\s*5rem 0;/s)
    expect(css).toMatch(/:root\[data-native-platform="darwin"\] \.topbar__leading \.brand-mark\s*{[^}]*display:\s*none;/s)
    expect(css).toMatch(/\.panel-resizer::after\s*{[^}]*background:\s*transparent;/s)
    expect(css).toMatch(/\.panel-resizer:hover::after,\s*\.panel-resizer:focus-visible::after,[^{]*{[^}]*background:\s*var\(--color-focus\);/s)
  })
  ...
  it('renders recursive sessions as a clear coordinator and child-session hierarchy', async () => {
    ...
    expect(css).toMatch(/\.outcome-review__row--branch\s*{[^}]*padding-inline-start:\s*calc\(var\(--outcome-branch-depth, 0\) \* 0\.75rem\);/s)
  ...
  it('keeps transcript metadata and the composer action inside narrow viewports', async () => {
    ...
    expect(narrowLayout).toMatch(/\.provider-setup-note__actions\s*{[^}]*grid-template-columns:\s*minmax\(0, 1fr\);/s)
    expect(narrowLayout).toMatch(/\.provider-setup-note__actions \.button\s*{[^}]*inline-size:\s*100%;/s)
    ...
    expect(css).toMatch(/\.provider-setup-note--handoff\s*{[^}]*background:\s*transparent;[^}]*border:\s*0;[^}]*box-shadow:\s*none;/s)
    ...
    expect(compactComposer).toMatch(/\.composer__toolbar,\s*\.composer__actions\s*{[^}]*align-items:\s*flex-start;[^}]*flex-wrap:\s*wrap;/s)
    expect(compactComposer).toMatch(/\.composer__connection\s*{[^}]*flex:\s*1 1 100%;[^}]*order:\s*-1;/s)
    expect(compactComposer).toMatch(/\.composer__primary-actions\s*{[^}]*max-inline-size:\s*100%;[^}]*margin-inline-start:\s*auto;/s)
    expect(compactComposer).toMatch(/\.composer__primary-actions \.button\s*{[^}]*max-inline-size:\s*100%;[^}]*min-inline-size:\s*0;[^}]*white-space:\s*normal;/s)
  ...
  it('preserves a readable thread title and primary composer actions at 320 CSS pixels', async () => {
    ...
    expect(minimumLayout).not.toMatch(/\.topbar__leading \.brand-mark\s*{[^}]*display:\s*none;/s)
  ...
  it('uses selection fill instead of decorative rails and gives the 390px inspector the full canvas', async () => {
    ...
    expect(css).not.toMatch(/\.message--user\s*{[^}]*border-inline-start/s)
    expect(css).not.toMatch(/\.(?:inline-loading|inline-error|models-loading|models-error)[^{]*{[^}]*border-inline-start/s)
